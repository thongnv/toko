...
async function fetchProductBySlug(url: string) {
  try {
...
export default function ProductRoute({ params }: { params: { slug: string } }) {
  ...
  const increaseOneProductItem = () => {
    setQuantity(quantity + 1);
  };

  const decreaseOneProductItem = () => {
  ...
  const price = (product: Product | undefined, quantity: number) =>
    product ? currencyFormat(product.attributes.price * quantity) : null;
    ...
    <div className="flex flex-wrap lg:flex-nowrap gap-3 lg:ml-4 lg:mx-4 mt-4">
      ...
      <div className="h-full grow md:flex md:flex-col gap-4">
        <div className="bg-white p-4 rounded-lg">
          ...
          <div className="text-2xl pb-4">{price(product, 1)}</div>
        ...
        <div className="bg-white p-4 rounded-lg">
          ...
          <div className="text-sm mb-2 w-96">{product?.attributes.description}</div>
          <div className="flex-none w-96 px-4 sm:px-6 md:px-0 overflow-hidden lg:overflow-auto scrollbar:!w-1.5 scrollbar:!h-1.5 scrollbar:bg-transparent scrollbar-track:!bg-slate-100 scrollbar-thumb:!rounded scrollbar-thumb:!bg-slate-300 scrollbar-track:!rounded dark:scrollbar-track:!bg-slate-500/[0.16] dark:scrollbar-thumb:!bg-slate-500/50">
      ...
      {/* cart */}
      <div className="h-full w-full lg:w-96 lg:block bg-white rounded-lg p-4">
        {/* <div className="mb-4">Tím, 128GB</div> */}
        <div className="text-sm font-bold mb-2">Số Lượng</div>
        <div className="flex gap-1 mb-4">
          <button
            className="border rounded-lg px-3 py-1"
            onClick={() => decreaseOneProductItem()}
          >
            -
          </button>
          <button className="border rounded-lg px-4 py-1">{quantity}</button>
          <button
            className="border rounded-lg px-3 py-1"
            onClick={() => increaseOneProductItem()}
          >
            +
          </button>
        </div>
        ...
        {product && (
          <div className="text-2xl pb-4">{price(product, quantity)}</div>
        )}
